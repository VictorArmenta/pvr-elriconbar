...
class Stats extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        if ($this->user->role !== "admin" and $this->user->role !== "sales") {
            redirect('');
        }
    }
    ...
    public function index()
    {
       ...
       // $date = date("Y-m-d H:i:s");
        $year = date("Y");
        $TodaySalesMXN = Sale::find_by_sql("SELECT sum(total) AS sum FROM ck_ventas WHERE created_at = '$date' AND divisa = 'MXN'");
        $TodaySalesUSD = Sale::find_by_sql("SELECT sum(total) AS sum FROM ck_ventas WHERE created_at = '$date' AND divisa = 'USD'");
        $Top5product = Sale_item::find_by_sql("SELECT name, product_id, sum(qt) AS totalquantity FROM ck_det_venta WHERE DATE_FORMAT(date, '%Y') = $year GROUP BY product_id ORDER BY SUM(qt) DESC LIMIT 5");
        ...
        $this->view_data['Registers'] = Register::all();
        ...
        $this->view_data['TodaySalesMXN'] = number_format((float)$TodaySalesMXN[0]->sum, $this->setting->decimals, '.', '');
        $this->view_data['TodaySalesUSD'] = number_format((float)$TodaySalesUSD[0]->sum, $this->setting->decimals, '.', '');
