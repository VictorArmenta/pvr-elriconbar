...
class Productcontroller extends CI_Controller
{
	...
	public function modifystock2($id) /*jar*/
    {
      $warehouses = Warehouse::find('all');
      $stores = Store::find('all'); 
      ...
      $content .= '<div class="col-md-12"><table class="table table-striped"><thead><tr><th>'.label("Stores").'</th><th><b>'.label("Existence").'</b></th><th>'.label("Unit").'</th><th>'.label("Quantity").'</th><th width="30%">'.label("Size").'</th><th>'.label("Size_unit").'</th></tr></thead><tbody class="itemslist">';
          
      $stock = Stock::find('first', array('conditions' => array('store_id = ? AND product_id = ?', $store->id, $id)));
      $quantity  = 1;
   	  $quantity1 = $stock ? $stock->quantity : '0';
	  if ($quantity1 > 0) {
			   $content .= '<input type="hidden" id="stock1" name="stock1" value="'.$quantity1.'">';
			   $content .= '<input type="hidden" id="store1" name="store1" value="'.$store->id.'">';	
			   $content .= '<input type="hidden" id="productID1" name="productID1" value="'.$id.'">';
               $content .= '<tr><td>'.$store->name.'</td><td><b>'.$quantity1.'</b></td><td>'.$product->unit.'</td><td><input max="100" min="1" type="number" id="quantity1" name="quantity1" value="'.$quantity.'"></td><td>'.$product->size.'</td><td>'.$product->size_unit.'</td></tr>';
	   }
    ...
    public function Viewproduct($id)
    {
      ...
      if ($product->type == 2) {
         ...
         $content .= '<div class="row"><div class="col-md-12"><h1>'.label("combinations").'</h1></div><div class="col-md-12"><table class="table">
             <thead>
                <tr>
                   <th class="col-xs-6">'.label("ProductName").'</th>
                   <th class="col-xs-2">'.label("Quantity").'</th>
				   <th class="col-xs-2">'.label("UM").'</th>
             ...
             foreach ($combos as $combo) {
				...
				$stock = Stock::find('first', array('conditions' => array('store_id = ? AND product_id = ?', $this->session->userdata('store'), $prod->id)));
				if ($stock->price <> 0 ) $price = $stock->price;
				else  $price = $prod->price;

				$costo = $price* $combo->quantity ;
                $content .= '<tr>
                  <td><a href="javascript:void(0)" onclick="Viewproduct('.$combo->item_id.')">'.$prod->name.' ('. $prod->code .')</a></td>
                  <td><b>'. $combo->quantity.'</b></td>
				  <td><b>'. $prod->unit.'</b></td>
				  <td align="right"><b>'. number_format((float)$costo, $this->setting->decimals, '.', '').'</b></td>
               </tr>';
			   $costototal = $costototal + $costo;
             }
			 $content .= '<tr><td></td><td></td><td align="right"><b>TOTAL'." " . $this->setting->currency .':</b></td><td align="right"><b>'.number_format((float)$costototal, $this->setting->decimals, '.', '').'</b></td><tr>';
             $content .= '  </tbody>
   ...
   public function Viewproductcompra($id)
    {
         $purchase_items = Purchase_item::find('all', array('conditions' => array('	product_id = ?', $id)));
         $content .= '<div class="row"><div class="col-md-12"><h1>'.label("list_prod_purchases").'</h1></div><div class="col-md-12"><table class="table">
             <thead>
                <tr>
                   <th class="col-xs-2"><b>'.label("Fecha").'</b></th>
				   <th class="col-xs-3"><b>'.label("Supplier").'</b></th>
                   <th class="col-xs-1"><b>'.label("Quantity").'</b></th>
				   <th class="col-xs-2"><b>'.label("UM").'</b></th>
				   <th class="col-xs-2" align="right"><b>'.label("Cost").'</b></th>
				   <th class="col-xs-2" align="right"><b>'.label("Subtotal").'</b></th>
                </tr>
             </thead>
             <tbody>';
             ...
             foreach ($purchase_items as $purchase_item) {  
                ...
                $content .= '<tr>
				  ...
				  <td ><b>'. $prod->unit.'</b></td>
				  <td align="right"><font color="red"><b>'. number_format((float)$purchase_item->cost, $this->setting->decimals, '.', '').'</b></font></td>
				  <td align="right"><b>'. number_format((float)$purchase_item->subtotal, $this->setting->decimals, '.', '').'</b></td>
               </tr>'; 
			   $costototal = $costototal + $costo;
             } 
			 $content .= '<tr><td></td><td></td><td></td><td></td><td align="right"><b>TOTAL '." " . $this->setting->currency .':</b></td><td align="right"><b>'.number_format((float)$costototal, $this->setting->decimals, '.', '').'</b></td><tr>';
             $content .= '  </tbody>
           </table></div></div>';
   ...
   /******** hace que el producto sea invisible en una tienda *************/
