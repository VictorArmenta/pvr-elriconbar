...
class Invoices extends CI_Controller
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoice) {
           ...
           // $row[] = $invoice->discount;
			...
			/*JAR01 desahabilitar opciones que no funcionan php 7 actualmente se activaron
			 if ($this->user->role === "admin") 
			   $row[] = '<div class="btn-group"><a class="btn btn-primary" href="javascript:void(0)" dropdown-toggle" data-toggle="dropdown" >
            ...
            <i class="fa fa-ticket fa-fw" aria-hidden="true"></i> ' . label("Receipt") . '</a></li>

            <li class="divider"></li>
            <li><a href="javascript:void(0)" onclick="delete_invoice(' . "'" . $invoice->id . "'" . ')">
            <i class="fa fa-trash-o fa-fw" aria-hidden="true"></i> ' . label("Delete") . '</a></li></ul></div>';
			 else
				 $row[] = '<div class="btn-group"><a class="btn btn-primary" href="javascript:void(0)" dropdown-toggle" data-toggle="dropdown" >
			    <i class="fa fa-cog fa-fw"></i> ' . label("Action") . '</a>
				<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
				...
				<ul class="dropdown-menu">	
				<li><a href="javascript:void(0)" onclick="showTicket(' . "'" . $invoice->id . "'" . ')">
				<i class="fa fa-ticket fa-fw" aria-hidden="true"></i> ' . label("Receipt") . '</a></li></ul></div>'; */

            // agregar html para acción
            ...
            <a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
            <span class="fa fa-caret-down" title="Toggle dropdown menu"></span></a>
            <ul class="dropdown-menu"><li><a href="javascript:void(0)" onclick="Edit_Sale(' . "'" . $invoice->id . "'" . ')">
            <i class="fa fa-pencil fa-fw" aria-hidden="true"></i> ' . label("Edit") . '</a></li>

            <li><a href="javascript:void(0)" onclick="payaments(' . "'" . $invoice->id . "'" . ')">
            <i class="fa fa-credit-card-alt fa-fw" aria-hidden="true"></i> ' . label("Payements") . '</a></li>
    ...
    public function ShowTicket($id)
    {
	    ...
	    /*JAR Ajuste de ticket para version 7  actualmente no se usa
	    $this->db->from('ck_det_venta');     
	    $this->db->where('sale_id', $id);
	    $consultaDetalle = $this->db->get();
     
	    $this->db->from('ck_ventas');
        $this->db->where('id', $id);
        $consulta = $this->db->get(); 
		foreach ($consulta ->result() as $sale)
			{
        ...
        <div style="clear:both;">
		<span class="float-left">' . label("Date") . ': ' . date("d/m/Y", strtotime($sale->created_at)) . '</span>
        ...
        <table class="table" cellspacing="0" border="0"><thead><tr><th><em>#</em></th>
        <th style="width:180px; font-weight:bold;">' . label("Product") . '</th>
        <th style="width:65px; font-weight:bold;">' . label("Quantity") . '</th>
        <th style="text-align:right; width:85px; font-weight:bold;">' . label("SubTotal") . '</th></tr></thead>
	    <tbody>'; 
		
   	   
        $i = 1;
        foreach ($consultaDetalle ->result() as $posale)
			{
            ...
            <td style="text-align:right; width:85px; ">' . number_format((float)($posale->qt * $posale->price), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td></tr>';
            $i ++;
        }*/
        ...
        $ticket = '<div class="col-md-12">
        <div class="text-center">' . $this->setting->receiptheader . '</div>
        <div style="clear:both;" class="text-center"><b>
        ' . label("SaleNum") . '.: ' . sprintf("%05d", $sale->id) . '</b> 
        <div style="clear:both;">
        ...
        <table class="table" cellspacing="0" border="0" style="margin-bottom:8px;"><thead><tr><th><em>#</em></th>
        <th style="width:180px; font-weight:bold;">' . label("Product") . '</th>
        <th style="width:65px; font-weight:bold;">' . label("Quantity") . '</th>
        <th style="text-align:right; width:85px; font-weight:bold;">' . label("SubTotal") . '</th></tr></thead>
        <tbody>';
		...
		if (intval($sale->discount))
          ...
          <td style="text-align:left;">' . label("Discount") . " " . $sale->discount . '</td>
          <td style="text-align:right; padding-right:1.5%;font-weight:bold;">' . number_format((float)$sale->discountamount, $this->setting->decimals, '.', '') .'</td></tr>';
        
		if (intval($sale->tip))
            $ticket .= '<tr><td style="text-align:left;"></td>
          <td style="text-align:right; padding-right:1.5%;font-weight:bold;"></td>
          <td style="text-align:left; padding-left:1.5%;">' . label("Tip") . " ". $sale->tip . '</td>
          <td style="text-align:right;font-weight:bold;">' . number_format((float)$sale->tipamount, $this->setting->decimals, '.', '') .'</td>
		  </tr>';


        $ticket .= '<tr><td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("GrandTotal") . '</td><td colspan="2" style="border-top:1px dashed #000; padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)$sale->total, $this->setting->decimals, '.', '') .  '</td></tr><tr>';
        ...
        switch ($PayMethode[0]) {
            ...
            case '2': // case ckeck
                ...
                break;
            default:
                $ticket .= '<td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("Paid") . ' (' . label("Cash") . ')</td>
                <td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)$sale->firstpayement, $this->setting->decimals, '.', '') .  '</td></tr><tr>
                <td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("Change") . '</td>
                <td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)(floatval($sale->paid) - floatval($sale->total)), $this->setting->decimals, '.', '') .  '</td>
          ...
          foreach ($payements as $pay) {
             ...
             switch ($PayMethode[0]) {
                case '1': // case Credit Card
                    ...
                    break;
                case '2': // case ckeck
                    $ticket .= '<td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("ChequeNum") . ' (' . $PayMethode[1] . ')</td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">'.number_format((float)$pay->paid, $this->setting->decimals, '.', '').'</td></tr>';
                    break;
                default:
                    $ticket .= '<td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("Paid") . ' (' . label("Cash") . ')</td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">'. number_format((float)$pay->paid, $this->setting->decimals, '.', '') . '</td></tr>';
           }
          }
       } else{
          $ticket .= '</tbody></table>';
    ...
    public function showInvoice($id)
    {
        ...
        $ClientData = $client ? 'Cliente: ' . $client->name . '<br>Tel: '  . $client->phone . '<br>Email: ' . $client->email : label('WalkinCustomer');
/**********************************************/
        $ticket = '<div class="col-sm-12">
                        <table width="100%">
                            <tr>
                                <td align="left">
                                <span class="float-left">' . $this->setting->companyname . '<br>' . label("Tel") . ' ' . $this->setting->phone . '</span>
                                </td>
                                <td align="right"><img src="./files/Setting/' . $this->setting->logo . '" alt="" width="100px;" style="margin:15px;float:right;"/>
                                </td>
                            </tr>
                        </table>
                    </div>
        ...
        <h4 class="float-left">' .label("SaleNum"). ' # ' . sprintf("%05d", $sale->id) . '</h4> 
        <div style="clear:both;"></div>

        <span style="font-size:40px;font-weight:600;padding:5px;background-color:#415472;color:#fff;">' . label("INVOICE") . '</span><br><br><br>
        <div style="clear:both;"></div>
        ...
        <td align="right">
        <span style="margin-bottom:15px;float:right;width:100%;text-align:right;">' . $ClientData . '</span></td></tr>
        </table>
        ...
        <thead>
        <tr style="background-color:#555;color:#fff;font-weight:600"><th><em>#</em></th>
            <th>' . label("Product") . '</th>
...
/********************************/
        ...
        <div class="col-xs-4 col-xs-offset-8">
            <table class="table table-striped" cellspacing="0" border="0" style="margin:20px 0 30px 0;">
                <thead>
                    <tr>
                    <td style="text-align:left; padding:3px;">' . label("TotalItems") . '</td>
                    <td style="text-align:right; padding:3px; padding-right:1.5%;font-weight:bold;">' . $sale->totalitems . '</td>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                    <td style="text-align:left; padding:3px;">' . label("Total") . '</td>
                    <td style="text-align:right; padding:3px; padding-right:1.5%;font-weight:bold;">' . number_format((float)$sale->subtotal, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td>
                    </tr>';

        /*******************************/
        if (intval($sale->tax))
            $ticket .= '<tr>
        <td style="text-align:left; padding:3px; padding-left:1.5%;">' . label("tax") ." " . $sale->tax .'</td>
        <td style="text-align:right; padding:3px;font-weight:bold;">' . number_format((float)$sale->taxamount, $this->setting->decimals, '.', '') ." ". $this->setting->currency .'</td>
        </tr>';

		
		if (intval($sale->discount))
            $ticket .= '<tr>
                            <td style="text-align:left; padding:3px;">' . label("Discount") . " " .$sale->discount .'</td>
                            <td style="text-align:right; padding:3px; padding-right:1.5%;font-weight:bold;">' . number_format((float)$sale->discountamount, $this->setting->decimals, '.', '') . " ". $this->setting->currency . '</td>
                        </tr>';
        
		if (intval($sale->tip))
            $ticket .= '<tr>
        <td style="text-align:left; padding:3px; padding-left:1.5%;">' . label("tax") ." " . $sale->tip .'</td>
        <td style="text-align:right; padding:3px;font-weight:bold;">' . number_format((float)$sale->tipamount, $this->setting->decimals, '.', '') . " ". $this->setting->currency .'</td>
        </tr>';
		
		
		
        $ticket .= '<tr style="background-color:#415472;color:#fff;font-weight:600;font-size:20px;">
        <td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td>
        </tr>
        ';
        /*******************************/
...
/**********************************/
    ...
    public function payaments($id) {
      ...
      $sale = Sale::find($id);
      $change = ($sale->total - $sale->paid) > 0 ? ($sale->total - $sale->paid) : '';
      $content = '<div class="row"><div class="col-md-12"><h4><b>' . label("Total") . '</b> ' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . ' <b>&emsp;' . label("Paid") . ' :</b> ' . number_format((float)$sale->paid, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '<b> &emsp;' . label("Change") . ' :</b> ' . number_format((float)($sale->paid - $sale->total), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</h4></div></div>'; //($sale->paid - $sale->total) //no cambiar de orden

      $content .= '<div class="col-md-12">
      <table class="table">
          <thead>
              <tr><th width="20%">'.label("Date").'</th>
      ...
      if($payements){
         foreach ($payements as $pay) {
            ...
            <td>'; 
			if ($this->user->role === "admin") 
				$content .= '<a href="javascript:void(0)" onclick="deletepayement('.$pay->id.')"><i class="fa fa-trash" aria-hidden="true"></i></a>';
