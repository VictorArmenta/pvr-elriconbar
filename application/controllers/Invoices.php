...
class Invoices extends CI_Controller
{
    ...
    public function ShowTicket($id)
    {
       ...
       // $waiters = Waiter::find($sale->waiter_id); // nombre de cajero //captura id devuelve nombre
        ...
        <table class="table"  cellspacing="0" border="0"><thead><tr>
        <th style="font-weight:bold;">PRODUCTOS</th>
        <th style="font-weight:bold;">CANT</th>
        <th style="text-align:right; font-weight:bold;">IMPORTE</th></tr></thead>
        <tbody>';
        ...
        foreach ($posales as $posale) {
            ...
            <td style="text-align:right;  ">' . number_format((float)($posale->qt * $posale->price), $this->setting->decimals, '.', '') .  '</td></tr>';
            $i ++;
        }
        ...
        $height = 20;
        $width = 3;
        $ticket .= '<tr><td style="text-align:right;">' . $sale->totalitems . '</td><td style="text-align:left;">' . label("Total") . '</td><td style="text-align:right;font-weight:bold;">' . number_format((float)$sale->subtotal, $this->setting->decimals, '.', '') .'</td></tr>';
		...
		if (intval($sale->tip))
            $ticket .= '<tr>
          <td colspan="2" style="text-align:left; ">' . label("Tip") . " ". $sale->tip . '</td>
          <td style="text-align:right;font-weight:bold;">' . number_format((float)$sale->tipamount, $this->setting->decimals, '.', '') .'</td>
		  </tr>';


        $ticket .= '<tr><td colspan="2" style="text-align:left; font-weight:bold; ">TOTAL A PAGAR</td><td colspan="2" style="border-top:1px dashed #000; padding-top:5px; text-align:right; font-weight:bold;">' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . ' '.$sale->divisa.'</td></tr><tr>';

        $PayMethode = explode('~', $sale->paidmethod);
        switch ($PayMethode[0]) {
            case '1': // case Credit Card
                $ticket .= '<td colspan="2" style="text-align:left; font-weight:bold;">' . label("CreditCard") . '</td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">xxxx xxxx xxxx ' . substr($PayMethode[1], - 4) . '</td></tr><tr><td colspan="2" style="text-align:left; font-weight:bold; padding-top:5px;">' . label("CreditCardHold") . '</td><td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">' . $PayMethode[2] . '</td></tr>';
                break;
            case '2': // case ckeck
