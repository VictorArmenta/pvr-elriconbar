   ...
   <div class="row" style="margin-top:60px;">
      ...
      <div class="col-md-4">
         <div class="statCart Statcolor03">
   			...
   			<h2 style="display: inline"><span class="count"><?=$TodaySalesMXN;?></span> MXN </h2><br><h2 style="display: inline"><span class="count"> <?=$TodaySalesUSD;?></span> USD</h2><br>
...
<div class="panel panel-default">
    <div class="panel-body">
        <div class="row rangeStat" style="margin-top:50px;">
          <h3 class="col-sm-12"><i class="fa fa-money"></i> Cierres de caja <?/*=label('RegisterStats');*/?></h3>
          <div class="col-md-5">
...
<div class="panel panel-default">
    <div class="panel-body">
        <div class="row rangeStat" style="margin-top:50px;">
          <h3 class="col-sm-12"><i class="fa fa-money"></i> Productos por cierres de caja</h3>
          <div class="col-md-5">
             <div class="form-group">
                 <label for="customerSelect"><?=label('SelectStore');?></label>
                   <select class="js-select-options form-control" id="StoresSelect">
          ...
          <div class="col-md-2">
             <button class="cancelBtn btn btn-picker" type="button" onclick="getRegisterReportprod()"><?=label('GetReport');?></button>
          </div>
        </div>
    </div>
</div>
             ...
             <div class="form-group">
                 <label for="customerSelect"><?=label('SelectClient');?></label>
                   <select class="js-select-options form-control" id="customerSelect">
                      <option value="0"><?=label("Todos los clientes");?></option>
...
<div id="footer" style="background-color: #093457;width: 100%;">
  <div class="container">
    <p class="footer-block" style="margin: 20px 0;color:#fff;"><?=label('title');?> <?= $this->setting->companyname;?>. <?=label('version');?> <a target="_blank" href="http://www.pixelandhost.com/">Prosoft Restaurantes</a></p>
  </div>
</div>
   ...
   $(function() {
	  ...
	  $('input[name="daterangeDV"]').daterangepicker();
   ...
   function getDetalleVentaReport(id)  /*JAR*/
   {
      var store_id = $('#StoresSelect').find('option:selected').val();
     
           // ajax set data to database
        $.ajax({
            url : "<?php echo site_url('reports/getDetalleVentaReport') ?>/"+id,
            type: "POST",
...
function getDetalleVentaReportprod(id)  /*JAR*/
   {
      var store_id = $('#StoresSelect').find('option:selected').val();
     
           // ajax set data to database
        $.ajax({
            url : "<?php echo site_url('reports/getDetalleVentaReportprod') ?>/"+id,
            type: "POST",
			 data: {store_id: store_id},
            success: function(data)
            {
...
function getRegisterReportprod() /*JAR*/
   {
      var store_id = $('#StoresSelect').find('option:selected').val();
      var Range = $('#RegisterRange').val();
      var start = Range.slice(6,10)+'-'+Range.slice(0,2)+'-'+Range.slice(3,5);
      var end = Range.slice(19,23)+'-'+Range.slice(13,15)+'-'+Range.slice(16,18);
           ...
           // ajax set data to database
        $.ajax({
            url : "<?php echo site_url('reports/getRegisterReportprod')?>/",
            type: "POST",
            data: {store_id: store_id, start: start, end: end},
            success: function(data)
            {
